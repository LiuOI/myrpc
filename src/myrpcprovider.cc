...
void MyRpcProvider::Run() {
  // 读取配置文件rpcserver的信息
  std::string ip =
      MyrpcApplication::GetInstance().GetConfig().Load("rpcserverip");
  uint16_t port = atoi(MyrpcApplication::GetInstance()
                           .GetConfig()
                           .Load("rpcserverport")
                           .c_str());
  ...
  server.setConnectionCallback(
      std::bind(&MyRpcProvider::OnConnection, this, std::placeholders::_1));
  server.setMessageCallback(
      std::bind(&MyRpcProvider::OnMessage, this, std::placeholders::_1,
...
void MyRpcProvider::OnConnection(const TcpConnectionPtr &conn) {
  if (!conn->connected()) {
    // 和rpc client的连接断开了
    conn->shutdown();
  }
...
void MyRpcProvider::OnMessage(const TcpConnectionPtr &conn, Buffer *buffer,
                              Timestamp) {
  ...
  google::protobuf::Closure *done =
      google::protobuf::NewCallback<MyRpcProvider, const TcpConnectionPtr &,
...
void MyRpcProvider::SendRpcResponse(const TcpConnectionPtr &conn,
                                    google::protobuf::Message *response) {
